...
namespace lighting_map{
	...
	void lightingMap()
	{
		...
		if (image == NULL){
			std::cout << "ERROR::image  specular is null\n" << std::endl;
		}
		...
		// Specular map  //lighting_maps_specular_color.png   //container2_specular
		image = SOIL_load_image("/Users/cc_xueqin/programming/learning/opengl/project/learn_opengl/res/matrix.jpg", &width, &height, 0, SOIL_LOAD_RGB);
		if (image == NULL){
			std::cout << "ERROR::image is null\n" << std::endl;
		}
		...
		while (!glfwWindowShouldClose(window))
		{
			...
			glm::mat4 model;
			// add by xueqin
			GLfloat angle = glm::radians(glfwGetTime() * 60.0f);
